...
adv_model_dir = baseUrl + \
    "/pvqa_path.pth"
LogFileTrue = baseUrl+'/debug_true_%s.txt' % dataset_split
LogFileFalse = baseUrl+'/debug_false_%s.txt' % dataset_split
        ...
        for i, datum_tuple in enumerate(loader):
            ...
            with torch.no_grad():
                ...
                for qid, l, sentence, targ, imageId, in zip(ques_id, label.cpu().numpy(), sent, target_answers, img_id):
                    if(imageId in pathImages[dataset_split]):
                        ans = dset.label2ans[l]
    ...
    if(True):
        ...
        if(True):
            result = pvqa.evaluate(
                get_data_tuple(dataset_split, bs=valid_bs,
                               shuffle=False, drop_last=False),
